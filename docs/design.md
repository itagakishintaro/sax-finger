...
- オクターブの定義（記譜音、各オクターブはド〜シ）: 最低 = シ♭3・シ3のみ、低 = ド4〜シ4、中 = ド5〜シ5、高 = ド6〜ファ#6
- 音域外の組合せ（例: 最低のド、高いソ）はボタンを無効化する
- 異名同音（ド# = レ♭ など）はどちらからでも選択でき、同じ運指を表示する
- 選択中の音名を「低いソ#（= 低いラ♭）」の形式で表示する
...
│   ├── components/      # SaxDiagram(SVG), NoteSelector など
...
| 種類 | 場所 | 実行 | 内容 |
| --- | --- | --- | --- |
| 振る舞いテスト（BDD） | `features/` | `npm run test:e2e` | 「低いソを選ぶとL1L2L3が赤く表示される」等のシナリオ |
...
| 3 | MVP: 中央1オクターブ（ド4〜シ4の幹音）の選択 + SVG運指表示 |
| 4 | 全音域対応: オクターブ切替（最低・低・中・高）、最低音シ♭3・シ3〜最高音ファ#6 |
| 5 | #/♭対応: 変化記号の選択、異名同音、音域外の無効化 |
...
| #3 | 五線譜表示 | 添付イラストのように選択中の音を五線譜上にも表示する |
