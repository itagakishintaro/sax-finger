...
- 替え指のある音では運指トグル（[基本][サイドB♭]…）を表示し、切り替えられる。収録する替え指: シ♭4/5=サイドB♭・1&1、ド5/6=サイドC、ファ#4/5=替えF#（薬指）。フロントキー系の高音替え指は運指表と照合できるまで未収録
...
実装済み: #2 替え指表示（フロントキー系の高音替え指のみ未収録）、#3 五線譜表示（本体設計に昇格）、#4 PWA対応
