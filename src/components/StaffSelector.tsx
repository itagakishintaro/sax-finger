import { noteId, noteName, type Note, type PitchName } from '../domain/notes'
...
/** 五線譜上の縦位置。E4(第1線)をy=70とし、音が1つ上がるごとに5px上がる */
function noteY(note: Note): number {
  const diatonic = note.octave * 7 + PITCH_INDEX[note.pitch]
  const e4 = 4 * 7 + PITCH_INDEX.E
  return 70 - (diatonic - e4) * 5
}

/** 五線(y=30〜70)の外にある音に必要な加線のy座標 */
function ledgerYs(y: number): number[] {
  const ys: number[] = []
  for (let ledger = 80; ledger <= y; ledger += 10) ys.push(ledger)
  for (let ledger = 20; ledger >= y; ledger -= 10) ys.push(ledger)
  return ys
}
...
export function StaffSelector({ notes, selected, onSelect }: Props) {
  ...
  return (
    ...
    >
      {/* 五線 */}
      {STAFF_LINES.map((y) => (
        <line key={y} x1={8} y1={y} x2={width - 8} y2={y} stroke="currentColor" strokeWidth={1} />
      ))}
      {/* ト音記号(フォント未対応環境では省略表示になる) */}
      <text x={12} y={74} fontSize={58} fill="currentColor" aria-hidden>
        {'\u{1D11E}'}
      </text>
      {notes.map((note, i) => {
        ...
        return (
          ...
          >
            {/* タップ領域を広げる透明な帯 */}
            <rect x={x - NOTE_GAP / 2} y={0} width={NOTE_GAP} height={130} fill="transparent" />
            {ledgerYs(y).map((ly) => (
              <line
                key={ly}
                x1={x - 12}
